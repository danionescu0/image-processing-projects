...
class MotionDetector:
    def __init__(self, rezise_width: int, min_contour_area: int) -> None:
        self.__resize_width = rezise_width
    ...
    def get_motion_box(self, image):
        ...
        for c in countours[1]:
            ...
            y2.append(int(y + h))
        if len(x1) > 0:
            (p1x, p1y), (p2x, p2y) = self.__get_scaled_coordonate(((min(x1), min(y1)), (max(x2), max(y2))), initial_width)
            return image[p1y:p2y, p1x:p2x]

        return None

    def __get_scaled_coordonate(self, coordonates: tuple, initial_width: int) -> tuple:
        (p1x, p1y), (p2x, p2y) = coordonates
        scale_factor = int(initial_width / self.__resize_width)

        return ((p1x * scale_factor, p1y * scale_factor), (p2x * scale_factor, p2y * scale_factor))